...
instruction_t* decode_instructions(unsigned int* bytes, unsigned int num_instructions)
{
  ...
  instructions = (instruction_t*) malloc(sizeof(instruction_t)*num_instructions);
  for(i = 0; i<num_instructions; i++){
    current = instructions+i;
    raw_inst = *bytes;

    current->opcode = (raw_inst >> 27) & 0x1F;
    current->first_register = (raw_inst >> 22) & 0x1F;
    current->second_register = (raw_inst >> 17) & 0x1F;
    current->immediate = raw_inst & 0xFFFF;
